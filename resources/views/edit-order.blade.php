        ...
        <form method="POST" action="{{route('order.update', $order)}}">
            ...
            <div class="relative z-0 w-full mb-6 group">
                ...
                <select name="vehicle_id" id="vehicle_id" class="block py-2.5 px-4 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600" onchange="updatePrice()">
                    ...
                    <option value="{{ $vehicle->id }}" data-price="{{ $vehicle->price }}" {{ $vehicle->id == $order->vehicle_id ? 'selected' : '' }}>{{ $vehicle->model }}</option>
                    @endforeach
                </select>
            </div>
            <div class="relative z-0 w-full mb-6 group">
                <input type="number" name="order_count" id="model" value="{{ $order->order_count }}" class="block py-2.5 px-0 w-full text-sm text-black bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
            ...
            <div class="relative z-0 w-full mb-6 group">
                <input type="text" name="price" id="price" value="{{ $order->price }}" readonly class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
